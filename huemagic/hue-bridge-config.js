	...
	function HueBridge(config) {
		...
		this.pushUpdatedState = function (resource, updatedType, suppressMessage = false) {
			const msg = {
				...
				services: resource['services'] ? Object.keys(resource['services']) : [],
			...
			if (this.resources['_groupsOf'][resource.id]) {
				for (let g = this.resources['_groupsOf'][resource.id].length - 1; g >= 0; g--) {
		...
		this.patchQueue = fastq(
			function ({ type, id, patch, version }, callback) {
				// GET SERVICE ID
		...
		this.autoUpdateFirmware = function () {
			if ((config.autoupdates && config.autoupdates == true) || typeof config.autoupdates == 'undefined') {
				if (scope.firmwareUpdateTimeout !== null) {
					clearTimeout(scope.firmwareUpdateTimeout);
				}
				API.request({
					config: config,
					method: 'PUT',
					...
					.then(function (status) {
						if (scope.nodeActive == true) {
							scope.firmwareUpdateTimeout = setTimeout(function () {
								scope.autoUpdateFirmware();
							}, 60000 * 720);
						}
					})
		...
		axios({
			...
			.then(function (response) {
				...
				for (let i = response.data.length - 1; i >= 0; i--) {
					let ipAddress = response.data[i].internalipaddress;
