...
class HueBrightnessMessage {
	...
	constructor(resource, options = {}) {
		...
		const connectivity = resource.services.zigbee_connectivity
			? Object.values(resource.services.zigbee_connectivity)[0]
			: resource.services.zgp_connectivity
			? Object.values(resource.services.zgp_connectivity)[0]
			: false;

		let realLUX = service.light.light_level - 1;
...
class HueGroupMessage {
	constructor(resource, options = {}) {
		...
		for (const [type, _resources] of Object.entries(resource['services'])) {
	...
	constructor(resource, options = {}) {
		...
		if (service['color']) {
			...
			if (options.colornames == true) {
				let cNamesArray = colorUtils.colornamer(colorUtils.rgbHex(RGB.r, RGB.g, RGB.b));
		...
		if (service['gradient']) {
			...
			for (let gradientColor in service['gradient']['points']) {
				let gradientColorRGB = colorUtils.xyBriToRgb(
					gradientColor.color.xy.x,
					gradientColor.color.xy.y,
					gradientColor.dimming ? service.dimming.brightness : 100
				);
...
class HueTemperatureMessage {
	...
	constructor(resource, options = {}) {
		...
		let deviceValue = service.temperature.temperature;
		let celsius = Math.round(deviceValue * 100) / 100;
