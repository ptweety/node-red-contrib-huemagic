module.exports = function (RED) {
	...
	function HueScene(config) {
		...
		this.targetGroups = {};
		for (let w = scope.wires[0].length - 1; w >= 0; w--) {
			let oneWiredNode = RED.nodes.getNode(scope.wires[0][w]);
			if (
				oneWiredNode &&
				oneWiredNode.type == 'hue-group' &&
				oneWiredNode.exportedConfig &&
				oneWiredNode.exportedConfig.groupid &&
				oneWiredNode.exportedConfig.groupid.length > 1
			) {
		...
		this.on('input', function (msg, send, done) {
			...
			if (copyOfTargetGroups.length > 0) {
				...
				for (let g = copyOfTargetGroups.length - 1; g >= 0; g--) {
					...
					if (targetGroup) {
						...
						async.retry(
							...
							function (callback, results) {
								bridge
									.patch('group', targetGroup.info.idV1 + '/action', { scene: targetSceneID }, 1)
									.then(function () {
										callback(null, true);
									})
				...
				async.retry(
					...
					function (callback, results) {
						bridge
							.patch('scene', sceneDef, patchObject)
							.then(function () {
