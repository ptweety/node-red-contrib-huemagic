				...
				if (config.sensorid) {
					if (currentState.payload.reachable == false) {
						...
						if (currentState.payload.dark) {
							statusMessage = RED._('hue-brightness.node.lux-dark', { lux: currentState.payload.lux });
							scope.status({ fill: 'blue', shape: 'dot', text: statusMessage });
						} else if (currentState.payload.daylight) {
							statusMessage = RED._('hue-brightness.node.lux-daylight', { lux: currentState.payload.lux });
							scope.status({ fill: 'yellow', shape: 'dot', text: statusMessage });
						} else {
							statusMessage = RED._('hue-brightness.node.lux', { lux: currentState.payload.lux });
			...
			if (Object.values(patchObject).length > 0) {
				...
				async.retry(
					...
					function (callback, results) {
						bridge
							.patch('light_level', tempSensorID, patchObject)
							.then(function () {
								callback(null, true);
							})
